    ...
    <div id="content">
          ...
          <div class="card-body">
            <div class="admin-number-layout">
              ...
              <div class="quick-actions">
                ...
                <button class="btn btn-primary" onclick="adminSaveCurrentNumber()"><svg viewBox="0 0 24 24"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>Save Number</button>
      ...
      <div class="section" id="tab-giftcards">
        <div class="card">
          ...
          <div class="card-body">
            <div class="form-grid c3">
              <div class="form-group"><label>Task Title</label><input id="task-title" placeholder="Join AnneBella Channel"/></div>
              <div class="form-group"><label>Task Type</label><select id="task-type"><option value="channel">Channel Join</option><option value="group">Group Join</option><option value="community">Community</option><option value="custom">Custom Link</option></select></div>
              <div class="form-group"><label>Reward Credits</label><input id="task-reward" type="number" placeholder="0"/></div>
              <div class="form-group" style="grid-column:1/-1"><label>Task URL</label><input id="task-url" placeholder="https://t.me/channelname or https://whatsapp.com/channel/..."/></div>
              <div class="form-group" style="grid-column:1/-1"><label>Description</label><textarea id="task-desc" placeholder="Tell users what they need to join or complete"></textarea></div>
            </div>
            <button class="btn btn-success btn-full" onclick="createTask()"><span id="task-spin" style="display:none" class="spin"></span><svg viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Create Task</button>
          </div>
        </div>
        <div class="card">
          <div class="card-header"><span class="card-title"><svg viewBox="0 0 24 24"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>Mini App Tasks</span><button class="btn btn-primary btn-sm" onclick="loadTasks()"><svg viewBox="0 0 24 24"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg></button></div>
          <div class="card-body" style="padding:0"><div class="tbl-wrap"><table><thead><tr><th>Title</th><th>Type</th><th>Reward</th><th>Status</th><th>Action</th></tr></thead><tbody id="task-tbody"><tr><td colspan="5" class="empty">Loading…</td></tr></tbody></table></div></div>
        </div>
        <div class="card">
...
function adminSaveNumber(d){if(!d||!d.deviceId||!d.panelId)return;const saved=adminSavedNumbers().filter(x=>!(x.deviceId===d.deviceId&&x.panelId===d.panelId));saved.unshift({...d,savedAt:new Date().toISOString()});localStorage.setItem('annebella-admin-saved-numbers',JSON.stringify(saved.slice(0,30)));adminRenderSavedNumbers(false);}
function adminSaveCurrentNumber(){if(!adminCurrentNumber){toast('Select a number first',false);return;}adminSaveNumber(adminCurrentNumber);toast('Number saved');}
...
async function deleteTask(id){if(!confirm('Delete this task?'))return;try{const r=await fetch(B+'/api/tasks/'+id,{method:'DELETE'});if(r.ok){toast('Task deleted');loadTasks();}else toast('Task delete failed',false);}catch{toast('Network error',false);}}
