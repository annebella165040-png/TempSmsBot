...
<div class="wrap">
  ...
  <main class="page" id="task-page">
    <div class="card"><div class="title">Join Community</div><div class="actions"><button onclick="openLink('https://t.me/indiagates')">AnneBella</button><button class="blue" onclick="openLink('https://t.me/annebellapanel')">Updates</button><button class="blue" onclick="openLink('https://t.me/AnneBellaForums')">Forum</button><button class="red" onclick="openBot('support')">Support</button><button onclick="openLink('https://whatsapp.com/channel/0029VbD7cfT8aKvBVv2mOX0w')">WA Channel</button><button class="blue" onclick="openLink('https://chat.whatsapp.com/EipHVdNQvtuLhXxn6lA1N4')">WA Group</button></div></div>
    <div class="card"><div class="title">Tasks</div><div class="actions"><button onclick="openBot('refer')">Refer Friend</button><button class="blue" onclick="openBot('buy_credit')">Buy Credits</button><button class="blue" onclick="openBot('gift_card')">Gift Card</button><button class="red" onclick="openBot('support')">Support</button></div><div class="list" id="task-list" style="margin-top:10px"><div class="empty">Loading tasks...</div></div></div>
...
function openLink(url){tg?.openLink?tg.openLink(url):location.href=url}
function showPage(p){document.querySelectorAll('.page').forEach(x=>x.classList.toggle('active',x.id===p+'-page'));document.querySelectorAll('.nav button').forEach(x=>x.classList.toggle('active',x.dataset.page===p));if(p==='number')renderSaved();if(p==='task')loadTasks();if(p==='profile')renderProfile()}
...
async function loadTasks(){try{const tasks=await (await fetch('/api/tasks?active=1',{cache:'no-store'})).json();const box=qs('task-list');if(!box)return;if(!tasks.length){box.innerHTML='<div class="empty">No admin tasks available</div>';return}box.innerHTML=tasks.map((t,i)=>'<div class="row"><div><div class="mono">'+esc(t.title)+'</div><div class="small">'+esc(t.description||t.taskType)+' '+(t.rewardCredits?'- '+esc(t.rewardCredits)+' credits':'')+'</div></div><button onclick="openTask('+i+')">Open</button></div>').join('');window._miniTasks=tasks}catch{const box=qs('task-list');if(box)box.innerHTML='<div class="empty">Tasks load failed</div>'}}
function openTask(i){const t=(window._miniTasks||[])[i];if(t?.url)openLink(t.url)}
...
function useNumber(d){current=d;qs('number-detail').innerHTML=detail(d);qs('send-to').value=d.number||'';smsCache=[];qs('sms-list').innerHTML='<div class="empty">Load SMS after selecting number</div>'}
function saveCurrentNumber(){if(!current)return alert('Select number first');saveNumber(current);alert('Number saved')}
...
loadProfile();renderSaved();loadTasks();
</script>
</body>
</html>`;
